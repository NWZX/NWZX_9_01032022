...
export default class Login {
   ...
   * @param {string} param0.PREVIOUS_LOCATION
   * @param {import("../app/Store").default} param0.store
   * @param {Storage} param0.localStorage
   */
  constructor({ document: l_document, localStorage: l_localStorage, onNavigate, PREVIOUS_LOCATION: L_PREVIOUS_LOCATION, store }) {
    this.document = l_document
    this.localStorage = l_localStorage
    this.onNavigate = onNavigate
    this.PREVIOUS_LOCATION = L_PREVIOUS_LOCATION
    this.store = store
  ...
  login = (user) => {
    if (this.store) {
      ...
      return Promise.reject('store is not defined')
    }
  }
